...
def assumed_fill(game: Game) -> tuple[bool]:
    ...
    while fill_algorithm.count_items_remaining():
        ...
        if fill_algorithm.count_items_remaining() == 0:
            ...
            completable = game.all_locations["Morph Ball 43"]["item"] == Items.Morph
            #completable = True
            if completable:
...
def write_rom(game: Game, romWriter: Optional[RomWriter] = None) -> str:
    ...
    rom_name = f"Verticality{game.seed}.sfc"
    rom1_path = f"roms/{rom_name}"
    rom_clean_path = "roms/Verticality1.3.sfc"
    ...
    # Kraid door fix
    romWriter.writeBytes(0x1b16e, b"\xD9\xD0\x00\x05\x1E\x06\x01\x00\x00\x80\x00\x00")    
    # Fix glider ASM
    romWriter.writeBytes(0x78c50, b"\x00")
    # Fix glider item position
    romWriter.writeBytes(0x7979c, b"\x03\x0b")
